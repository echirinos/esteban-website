...
const bestFitConversations = [
  ...
  },
  {
    ...
    detail:
      "Build the reference apps, docs, onboarding paths, and support loops that make a platform easier to adopt.",
  },
  {
      ...
      <section className="grid gap-8 py-6 md:min-h-[calc(100svh-8rem)] md:grid-cols-[minmax(0,1fr)_360px] md:items-center md:py-10 lg:grid-cols-[1.04fr_0.96fr]">
        <div>
          ...
          <p className="mt-5 max-w-2xl leading-relaxed text-base-content/64">
            Senior Technical Solutions Engineer at Coinbase. Previously at TRM
            Labs, Polygon Labs, OpenSea, Google, Microsoft, and JPMorgan Chase
            across developer platforms, cloud, fintech, product feedback loops,
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {portfolioMetrics.slice(0, 3).map((metric) => (
              <div
                key={metric.label}
                className="rounded-lg border border-base-content/10 bg-base-100/70 px-4 py-3"
              >
                <p className="text-2xl font-bold text-primary">{metric.value}</p>
                <p className="mt-1 text-xs font-semibold leading-snug text-base-content/60">
                  {metric.label}
                </p>
              </div>
            ))}
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-3 max-w-64 text-3xl font-bold leading-[1.05] text-white drop-shadow-[0_2px_14px_rgba(0,0,0,0.42)]">
                  Applied AI, DevEx, product signal, and demo engineering.
                </p>
              </div>
              <Image
